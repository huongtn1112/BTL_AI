    ...
    def train(self, dataset):
        ...
        while wrongPrediction:
            ...
            for x, y in dataset.iterate_once(batch_size):
                answer = nn.as_scalar(y)
                guess = self.get_prediction(x)
                if guess != answer:  # wrong guess
...
class RegressionModel(object):
    ...
    def run(self, x):
        ...
        """
        "*** YOUR CODE HERE ***"
        predictions = nn.AddBias(nn.Linear(
            nn.ReLU(nn.AddBias(nn.Linear((nn.ReLU(nn.AddBias(nn.Linear(x, self.m1), self.b1))), self.m2), self.b2)),
            self.m3), self.b3)
        return predictions
    ...
    def train(self, dataset):
        ...
        while True:
            for x, y in dataset.iterate_once(1):
                ...
                grad_m1, grad_b1, grad_m2, grad_b2, grad_m3, grad_b3 = nn.gradients(loss,
                                                                                    [self.m1, self.b1, self.m2, self.b2,
                                                                                     self.m3, self.b3])
                self.m1.update(grad_m1, -0.01)
                self.b1.update(grad_b1, -0.01)
                self.m2.update(grad_m2, -0.01)
                self.b2.update(grad_b2, -0.01)
                self.m3.update(grad_m3, -0.01)
                self.b3.update(grad_b3, -0.01)
            all_loss = self.get_loss(nn.Constant(dataset.x), nn.Constant(dataset.y))
            if nn.as_scalar(all_loss) <= 0.02:
                return
    ...
    def train(self, dataset):
        ...
        while True:
            for x, y in dataset.iterate_once(self.batch_size):
                loss = self.get_loss(x, y)
                grads = nn.gradients(loss, [self.m1, self.b1, self.m2, self.b2, self.m3, self.b3, self.m4, self.b4])
                self.m1.update(grads[0], -self.learning_rate)
                self.b1.update(grads[1], -self.learning_rate)
                self.m2.update(grads[2], -self.learning_rate)
                self.b2.update(grads[3], -self.learning_rate)
                self.m3.update(grads[4], -self.learning_rate)
